<template>
	<div class="RobotDetails_body" @mouseenter="check_log" >
		<div class="body-mains" style="margin-top: 80px;">
			<div class="body-left col-lg-12 col-xs-12 col-sm-12" style="width: 100%;height: auto; overflow: hidden;padding: 0;">
				<div class="author_info">
					<!--<div class="authorinfo_top" style="margin-bottom: 30px;">
						<div class="author_pic">
							<img :src="mdImg1" alt="" />
						</div>
						<div class="lg-width" style="margin-left: 32px;width: 800px;">

							<span class="author_til"> {{robotInfo.Name}} </span>

							<p style="line-height:22px;font-size: 14px;color: #999999;margin-top: 5px;">
								<span>
									<img :src="T2" style="width: 20px;border-radius: 10px;margin-top: -4px;margin-right: 5px;"><span style="color:#000;margin-right: 10px;">{{robotInfo.Owner}}</span>
								</span>
							</p>
							<p style="line-height:22px;font-size: 14px;color: #000;margin-top: 15px;">
								{{$t("netnew.Description") }}:{{robotBasicInfo.description}}
							</p>

						</div>
						<div class="lg-width" style="margin-top: 18px;float: right;margin-right: 12px;">
							<a v-if="ifLogin" href="#buy_now" @click="getBalance(item,'1')" data-toggle="modal" class="lsy_buy">{{$t("robotdetails.Subscribenow") }}</a>
							<a v-if="!ifLogin" @click="login" data-toggle="modal" class="lsy_buy">{{$t("robotdetails.Subscribenow") }}</a>
						</div>
					</div>-->
				</div>
					...
					<div style="height: 100px" class="author_b2">
						<table class="col-lg-7 robtD" style="height: 70px;">
							<tr style="font-size: 15px;color: #444;padding-left: 15px;">
								<td style="padding: 20px;" class="mdbox-title"> {{$t("robotdetails.Transactionpairs") }}</td>
								<!--<td>{{$t("robotdetails.Usagestrategy") }}</td>-->
								<td style="color: #909499;" class="mdbox-title">{{$t("robotdetails.Standardcurrency") }}</td>
							</tr>
							<tr style="color: #999;font-size: 14px;">
								<td style="padding-left: 20px;" class="mdbox-text">{{robotInfo.Pair}} </td>
								<td class="mdbox-text">{{robotInfo.BaseCurrency}} </td>
							</tr>
						</table>
					</div>
				</div>
				...
				<div class="robot-basice-info">
					<span class="author_js">&nbsp;&nbsp;{{$t("netnew.BasicInformation")}}</span>
					<div>
						<section>
							<div >
								<table class="table" style="text-align: center">
									<thead>
										<tr class="mdbox-title">
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.RobotName")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.TradeType")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.RobotRange")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.OperationFrequency")}}</th>
										</tr>
									</thead>
									<tbody>
									<tr class="robot-info-text" >
										<td v-show="$i18n.locale!='zh-CN'">{{ robotBasicInfo.basicData.Name }}</td>
									...
									<tr class="robot-info-text" >
										...
										<td v-show="$i18n.locale!='zh-CN'" colspan="3">{{ robotBasicInfo.basicData.Introduction }}</td>
				...
				<div class="robot-component-info">
					<span class="author_js">&nbsp;&nbsp;{{$t("netnew.ComponentInfo")}}</span>
					<div>
						<section>
							<div>
								<table class="table " style="text-align: center">
									<thead>
										<tr class="mdbox-title">
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.RiskManagement")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.RobotWeighting")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.FundOptimization")}}</th>
											<th style="font-weight: 500 ;text-align: left!important;">{{$t("netnew.OrderExecution")}}</th>
										</tr>
									</thead>
									<tbody>
										<tr class="robot-info-text" >
				...
				<div class="tbandtime" >
					<div class="lg-width" style="width: 100%;text-align: left;margin-bottom: 0;">
						<div>
							<span class="author_js" style="width: auto;display: inline-block;">&nbsp;&nbsp; {{ $t("robotdetails.cumulative") }} </span>
							...
							<table class="table" style="text-align: center;">
								<thead>
									<tr style="color: #fff;font-size: 18px;">
										...
										<th style="text-align: left !important;">{{$t("robotdetails.Buyingprice") }} (NET)</th>
								...
								<tbody>
									<tr v-for="(robot_owner,index) in robotOwners" style="color: #fff;">
										<td style="font-size: 18px;color: #fff;">{{ robot_owner.nickName}}</td>
										<td style="font-size: 18px;color: #fff;">{{ robot_owner.buyTime | timer}}</td>
										<td style="font-size: 18px;color: #fff;">{{ robot_owner.buyPrice }}</td>
										<td style="font-size: 18px;color: #fff;">{{ robot_owner.unitPrice }}</td>
									</tr>
								</tbody>
							</table>
						</div>
...
//				this.robotOrder(),
		...
		methods: {
			...
			check_waring() {
				...
				if($(".childenop").val() == null) {
					...
					if(this.checkWaring == true) {
						$(".card-btm").attr("disabled", false)
						$(".card-btm").css("background-color", "#055ccb")
					} else {
						$(".card-btm").css("background-color", "#dcdcdc")
						$(".card-btm").attr("disabled", true)
					}
			...
			getMoreInfo() {
				...
				this.getMorePage(this.pageInfo.pageNum);
			},
			getMorePage(pageNum) {
				...
				robotOwner(this.pageInfo).then(data => {
					if(data.message == "success") {
						for(var i = 0; i < data.data.list.length; i++) {
							this.robotOwners.push(data.data.list[i]);
						}
